    ...
    if chart_mode == "📊 Standard Charts":
        ...
        if st.button("🚀 Generate Top N Chart", key="gen_top"):
            ...
            try:
                ...
                color_palette = px.colors.qualitative.Set2  # You can try Set1, Pastel1, etc.
                color_args = {}

                if color_column:
                    if pd.api.types.is_object_dtype(data[color_column]) or pd.api.types.is_categorical_dtype(data[color_column]):
                        color_args = {"color": color_column,"color_discrete_sequence": color_palette}
                    else:
                        color_args = {"color": color_column,"color_continuous_scale": "viridis"}
                # Step 4: Generate selected chart type
                fig = None

                # Generate based on selected chart type
                if top_chart_type == "bar":
                    fig = px.bar(grouped, x=cat_col, y=val_col, color=color_column or val_col, **color_args)
                elif top_chart_type == "horizontal_bar":
                    fig = px.bar(grouped, x=val_col, y=cat_col, orientation='h', color=color_column or val_col, **color_args)
                    ...
                    if pd.api.types.is_numeric_dtype(grouped[cat_col]) or pd.api.types.is_datetime64_any_dtype(grouped[cat_col]):
                       fig = px.line(grouped.sort_values(cat_col),x=cat_col,y=val_col,color=color_column or None,markers=True,**color_args)
                    else:
                    ...
                    fig = px.scatter(grouped, x=cat_col, y=val_col, size=val_col, color=color_column or cat_col,**color_args)  # <-- RAW data
                elif top_chart_type == "box":
                    fig = px.box(filtered_data, x=cat_col, y=val_col, color=color_column or cat_col,**color_args)  # <-- RAW data
                else:
                    raise ValueError("Unsupported chart type selected.")
